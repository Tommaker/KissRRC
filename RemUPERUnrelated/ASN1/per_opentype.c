...
uper_open_type_get_simple(asn_codec_ctx_t *ctx, asn_TYPE_descriptor_t *td,
	...
	do {
		...
		if(bufLen + chunk_bytes > bufSize) {
            ...
            // Add Input value of buffer length, Cause cannot get the buffer length by Memory Pool scheme
...
int
uper_open_type_skip_padding_bits(asn_per_data_t *pd, int skip_nbits) {
    return per_skip_bits(pd, skip_nbits);
}
...
uper_ugot_refill(asn_per_data_t *pd) {
	...
	if(next_chunk_bytes == 0) {
		...
		ASN1_ASSERT(!arg->repeat);	/* Implementation guarantee */
	}
